  ...
  emptyList(),
          ...
          val resolvedMimeType = mimeTypeOverride ?: inferMimeType(url = url,
  filename = null)
          ...
          val extractorsFactory = customExtractorsFactory ?:
  DefaultExtractorsFactory()
          val defaultFactory = DefaultMediaSourceFactory(httpDataSourceFactory,
  extractorsFactory).apply {
  ...
  customSubtitleParserFactory != null

          // Sidecar subtitles are more reliable through
  ...
  DashMediaSource.Factory(httpDataSourceFactory)
      ...
      companion object {
          ...
          fun sanitizeHeaders(headers: Map<String, String>?): Map<String,
  String> {
  ...
  URLDecoder.decode(parts[1], "UTF-8")
          ...
          internal fun sniffManifestMimeType(snippet: String?): String? {
              ...
              return when {
                  ...
                  normalized.startsWith("<?xml") && normalized.contains("<mpd")
  -> MimeTypes.APPLICATION_MPD
          ...
          private fun probeMimeTypeWithHead(url: String, headers: Map<String,
  String>): String? {
              val connection = openConnection(url = url, headers = headers,
  method = "HEAD")
              return try {
                  connection.responseCode
                  normalizeMimeType(connection.contentType)
                      ?: inferMimeType(url =
  connection.url?.toString().orEmpty(), filename = null)
              } catch (_: Exception) {
                  null
              } finally {
          ...
          private fun probeMimeTypeWithRangeGet(url: String, headers:

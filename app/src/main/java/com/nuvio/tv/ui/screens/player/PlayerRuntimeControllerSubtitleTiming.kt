...
private fun PlayerRuntimeController.executeSubtitleDownload(url: String): String {
    ...
    subtitleAutoSyncHttpClient.newCall(requestBuilder.build()).execute().use { response ->
        ...
        // Read raw bytes instead of response.body?.string(): OkHttp's string() decodes
        // using the response's declared Content-Type charset and silently falls back to
        // UTF-8 when none is declared (the common case for .srt/.vtt hosts) — mangling
        // subtitles authored in a legacy codepage (e.g. Windows-1255 for Hebrew).
        // SubtitleCharsetDetector runs real charset detection on the raw bytes instead.

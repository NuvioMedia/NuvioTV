...
fun SourceStatusFilterChip(
    ...
    val textColor = when {
        isError -> OmnioColors.Error
        isFocused || isSelected -> OmnioColors.OnSecondary
        else -> OmnioColors.TextSecondary
    }
    ...
    FilterChip(
        ...
        colors = FilterChipDefaults.colors(
            containerColor = if (shouldUseNormalColors) OmnioColors.BackgroundCard else OmnioColors.Error.copy(alpha = 0.05f),
            focusedContainerColor = if (shouldUseNormalColors) OmnioColors.Secondary else OmnioColors.Error.copy(alpha = 0.08f),
            selectedContainerColor = if (shouldUseNormalColors) OmnioColors.Secondary else OmnioColors.Error.copy(alpha = 0.07f),
            focusedSelectedContainerColor = if (shouldUseNormalColors) OmnioColors.Secondary else OmnioColors.Error.copy(alpha = 0.09f),
        ...
        border = FilterChipDefaults.border(
            border = Border(
                border = BorderStroke(1.dp, if (isError) OmnioColors.Error.copy(alpha = 0.7f) else OmnioColors.Border),
                shape = RoundedCornerShape(20.dp)
            ),
            focusedBorder = Border(
                border = BorderStroke(2.dp, if (isError) OmnioColors.Error.copy(alpha = 0.8f) else OmnioColors.FocusRing),
                shape = RoundedCornerShape(20.dp)
            ),
            selectedBorder = Border(
                border = BorderStroke(1.dp, if (isError) OmnioColors.Error.copy(alpha = 0.75f) else OmnioColors.Primary),
                shape = RoundedCornerShape(20.dp)
            ),
        ...
        Row(
            ...
            if (isLoading) {
                MaterialCircularProgressIndicator(
                    ...
                    color = if (isFocused || isSelected) OmnioColors.OnSecondary else OmnioColors.TextSecondary,
                    strokeWidth = 1.5.dp
                )
            }
            Text(
